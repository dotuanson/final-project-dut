...
def supervised_training_iter(
    ...
    # write_image(image=boundaries, name="boundaries.png")  # Write trimap

    # calculate the semantic loss
    gt_semantic = F.interpolate(gt_matte, scale_factor=1/16, mode='bilinear')
    gt_semantic = blurer(gt_semantic)
    """
        differ with paper:  not have 1/2
                            semantic_scale
    """
    ...
    # write_image(image=pred_boundary_detail, name="pred_boundary_detail.png")  # write pred_boundary_detail
    gt_detail = torch.where(boundaries, trimap, gt_matte)
    # write_image(image=gt_detail, name="gt_detail.png")  # write gt_detail
    detail_loss = torch.mean(F.l1_loss(pred_boundary_detail, gt_detail))

    # calculate the matte loss
    pred_boundary_matte = torch.where(boundaries, trimap, pred_matte)
    """
        differ from paper: 4.0 * F.l1_loss(pred_boundary_matte, gt_matte)
    ...
    """
        differ from paper: semantic_scale = 10.0 x
                           detail_scale = 10.0
                           matte_scale = 1.0
    """
    loss = semantic_scale * semantic_loss + detail_scale * detail_loss + matte_scale * matte_loss
    loss.backward()
    optimizer.step()
...
def write_image(image, name):
    ...
    test = np.array(test)
    test = np.uint8(test[0][0])
    test[test == 1] = 255
    cv2.imwrite(name, test)